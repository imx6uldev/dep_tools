...
static struct btd_adapter *btd_adapter_new(uint16_t index)
{
	...
	if (hci_devinfo(adapter->dev_id, &info) < 0) {
		DBG("Can't get device info for hci%d\n",adapter->dev_id);
	}
	ba2str(&info.bdaddr, addr);
	sprintf(tmp_system_name, "%s_%s", main_opts.name, addr);
	...
	adapter->system_name = g_strdup(tmp_system_name);
	...
	adapter->discoverable_timeout = main_opts.discovto;
	adapter->pairable_timeout = main_opts.pairto;
	DBG("System name: %d", adapter->dev_id);
